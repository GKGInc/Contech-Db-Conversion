...
--  - Changed [desc] from [text] to [varchar](2000)
	...
	declare @sql_class varchar(4000);
	...
	BEGIN
		...
		BEGIN		
			SELECT @sql_class = 'ALTER TABLE ' +  OBJECT_SCHEMA_NAME(k.parent_object_id) + '.[' + OBJECT_NAME(k.parent_object_id) + '] DROP CONSTRAINT ' + k.name FROM sys.foreign_keys k WHERE referenced_object_id = object_id('class')
			PRINT (@sql_class)	
			EXEC (@sql_class)
		END
...
--  - Changed [doctypesid] to [doctypeid]
	...
	declare @sql_doctypes varchar(4000);
	...
	BEGIN
		...
		BEGIN		
			SELECT @sql_doctypes = 'ALTER TABLE ' +  OBJECT_SCHEMA_NAME(k.parent_object_id) + '.[' + OBJECT_NAME(k.parent_object_id) + '] DROP CONSTRAINT ' + k.name FROM sys.foreign_keys k WHERE referenced_object_id = object_id('doctypes')
			PRINT (@sql_doctypes)	
			EXEC (@sql_doctypes)
		END
...
--	- [docs_hdr].[doctype] --> [doctypeid]	-- FK = [doctypes].[doctype] --> [doctypes].[doctypeid]
	...
	declare @sql_docs_hdr varchar(4000);
	...
	BEGIN
		...
		BEGIN		
			SELECT @sql_docs_hdr = 'ALTER TABLE ' +  OBJECT_SCHEMA_NAME(k.parent_object_id) + '.[' + OBJECT_NAME(k.parent_object_id) + '] DROP CONSTRAINT ' + k.name FROM sys.foreign_keys k WHERE referenced_object_id = object_id('docs_hdr')
			PRINT (@sql_docs_hdr)	
			EXEC (@sql_docs_hdr)
		END
...
--	- [docs_dtl].[rev_emp] --> [rev_employeeid]	-- FK = [employee].[empnumber] -> [employee].[employeeid]
	...
	declare @sql_docs_dtl varchar(4000);
	...
	BEGIN
		...
		BEGIN		
			SELECT @sql_docs_dtl = 'ALTER TABLE ' +  OBJECT_SCHEMA_NAME(k.parent_object_id) + '.[' + OBJECT_NAME(k.parent_object_id) + '] DROP CONSTRAINT ' + k.name FROM sys.foreign_keys k WHERE referenced_object_id = object_id('docs_dtl')
			PRINT (@sql_docs_dtl)	
			EXEC (@sql_docs_dtl)
		END
...
--	- [colorant].[document] --> [docs_dtlid]	-- FK = [docs_dtl].[document] -> [docs_dtl].[docs_dtlid]
	...
	declare @sql_colorant varchar(4000);
	...
	BEGIN
		...
		BEGIN		
			SELECT @sql_colorant = 'ALTER TABLE ' +  OBJECT_SCHEMA_NAME(k.parent_object_id) + '.[' + OBJECT_NAME(k.parent_object_id) + '] DROP CONSTRAINT ' + k.name FROM sys.foreign_keys k WHERE referenced_object_id = object_id('colorant')
			PRINT (@sql_colorant)	
			EXEC (@sql_colorant)
		END
...
--	- [componet].[class] --> [classid]			-- FK = [class].[class] -> [class].[classid]
	...
	--declare @sql_componet varchar(4000);
	...
	--	BEGIN		
	--		SELECT @sql_componet = 'ALTER TABLE ' +  OBJECT_SCHEMA_NAME(k.parent_object_id) + '.[' + OBJECT_NAME(k.parent_object_id) + '] DROP CONSTRAINT ' + k.name FROM sys.foreign_keys k WHERE referenced_object_id = object_id('componet')
	--		PRINT (@sql_componet)	
	--		EXEC (@sql_componet)			
	--	END
