...
SET IDENTITY_INSERT [shporder] ON;

INSERT INTO [shporder] ([shporderid],[orderid],[aropenid],[shipdate],[palletqty],[ship_via],[comments],[add_userid],[add_dt])
SELECT [rawUpsize_Contech].[dbo].[shporder].[shporderid]
  ...
  LEFT JOIN [orders] orders ON [rawUpsize_Contech].[dbo].[shporder].[job_no] = orders.[job_no]			-- FK = [orders].[job_no] --> [orders].[ordersid]
  LEFT JOIN [aropen] aropen ON [rawUpsize_Contech].[dbo].[shporder].[invoice_no] = aropen.[invoice_no]	-- FK = [aropen].[invoice_no] --> [aropen].[aropenid]
  LEFT JOIN [users] users ON [rawUpsize_Contech].[dbo].[shporder].[add_user] = users.[username]			-- FK = [users].[username] --> [users].[add_userid]
  
SET IDENTITY_INSERT [shporder] OFF;

--SELECT * FROM [shporder]
...
INSERT INTO [system] ([company],[address],[address2],[city],[state],[zip],[country],[phone],[fax],[email],[orderid],[customerid],[aropenid],[po_tax],[rollover],[tax],[vat],[paid_acct],[open_acct],[ass_acct],[cont_acct]
...
SELECT [rawUpsize_Contech].[dbo].[system].[company]
  ...
  LEFT JOIN [orders] orders ON [rawUpsize_Contech].[dbo].[system].[job_no] = orders.[job_no]			-- FK = [orders].[job_no] --> [orders].[ordersid]
  LEFT JOIN [customer] customer ON [rawUpsize_Contech].[dbo].[system].[cust_no] = customer.[cust_no]	-- FK = [customer].[cust_no] --> [customer].[customerid]
  LEFT JOIN [aropen] aropen ON [rawUpsize_Contech].[dbo].[system].[invoice_no] = aropen.[invoice_no]	-- FK = [aropen].[invoice_no] --> [aropen].[aropenid]
  LEFT JOIN [qrn] qrn ON [rawUpsize_Contech].[dbo].system.[qrn_no] = qrn.[qrn_no]						-- FK = [qrn].[qrn_no] --> [qrn].[qrnid]
  LEFT JOIN [corractn] corractn ON [rawUpsize_Contech].[dbo].[system].[car_no] = corractn.[car_no]		-- FK = [corractn].[car_no] --> [corractn].[corractnid]
...
INSERT INTO [turn_tbl] ([table],[tbl_od],[ct_cm],[mfg],[max_len],[comment])
...
SET IDENTITY_INSERT [turnmemo] ON;

INSERT INTO [turnmemo] ([turnmemoid],[turn_tblid],[memodate],[memouser],[memonote])
SELECT [rawUpsize_Contech].[dbo].[turnmemo].[turnmemoid]
  ...
  LEFT JOIN [turn_tbl] turn_tbl ON [rawUpsize_Contech].[dbo].[turnmemo].[table] = turn_tbl.[table]	
  LEFT JOIN [users] ON [rawUpsize_Contech].[dbo].[turnmemo].[memouser] = users.[username]	
  
SET IDENTITY_INSERT [turnmemo] OFF;

--SELECT * FROM [turnmemo]
