...
--  - Changed [mfg_locid] to be primary key
	...
	INSERT INTO [dbo].[mfg_loc] ([mfg_locid],[loc_description],[lot_suffix],[address],[address2],[city],[state],[country],[phone],[fax],[email],[zip],[customerid],[vendorid]) 
	SELECT mfg_loc.[mfg_locid]
		,mfg_loc.[loc_description]
		,mfg_loc.[lot_suffix]
		,mfg_loc.[address]
		,mfg_loc.[address2]
		,mfg_loc.[city]
	...
	FROM [rawUpsize_Contech].[dbo].[mfg_loc] mfg_loc
	LEFT JOIN [dbo].[customer] customer ON mfg_loc.[cust_no] = customer.[cust_no] 
	LEFT JOIN [dbo].[vendor] vendor ON mfg_loc.[ven_id] = vendor.[ven_id]	
	ORDER BY mfg_loc.[mfg_locid]
