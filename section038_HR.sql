...
SET IDENTITY_INSERT [lottranx] ON;

INSERT INTO [lottranx] ([lottranxid],[cmpcasesid],[old_caseid],[trx_qty],[trx_from],[trx_to],[trx_when],[userid],[mfg_locid])
SELECT [rawUpsize_Contech].[dbo].[lottranx].[lottranxid]
  ...
  LEFT JOIN [users] users ON [rawUpsize_Contech].[dbo].[lottranx].[userid] = users.[username]	-- FK = [users].[userid]
  
SET IDENTITY_INSERT [lottranx] OFF;

--SELECT * FROM [lottranx]
...
SET IDENTITY_INSERT [matlincr] ON;

INSERT INTO [matlincr] ([matlincrid],[po_hdrid],[ven_inv],[ven_cr_inv],[cr_amt],[amt_rec],[tot_amt],[date],[date_ret],[ct_cr_no],[memo],[status],[ship],[retur_auth],[dispos],[ct_lot],[qrnid],[ven_lot],[incoming],[componetid])
SELECT [rawUpsize_Contech].[dbo].[matlincr].[matcr_key]
  ...
  LEFT JOIN [po_hdr] po_hdr ON [rawUpsize_Contech].[dbo].[matlincr].[po_no] = po_hdr.[po_no]
  LEFT JOIN [qrn] qrn ON [rawUpsize_Contech].[dbo].[matlincr].[qrn_no] = qrn.[qrn_no] 
  LEFT JOIN [componet] componet ON [rawUpsize_Contech].[dbo].[matlincr].[comp] = componet.[comp] 
  
SET IDENTITY_INSERT [matlincr] OFF;

--SELECT * FROM [matlincr]
...
SET IDENTITY_INSERT [matrldoc] OFF;

--SELECT * FROM [matrldoc]
...
IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'dbo' AND  TABLE_NAME = 'newdocnotice'))
    DROP TABLE [newdocnotice]

CREATE TABLE [newdocnotice](
...
SET IDENTITY_INSERT [newdocnotice] ON;

INSERT INTO [newdocnotice] ([newdocnoticeid],[userid],[docs_dtlid],[add_dt],[dismiss_dt])
SELECT [rawUpsize_Contech].[dbo].[newdocnotice].[pk]
  ...
  LEFT JOIN [users] users ON [rawUpsize_Contech].[dbo].[newdocnotice].[userid] = users.[username]	-- FK = [users].[userid]

SET IDENTITY_INSERT [newdocnotice] OFF;

--SELECT * FROM [newdocnotice]
