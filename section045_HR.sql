...
--SET IDENTITY_INSERT [userrmnd] ON;

INSERT INTO [userrmnd] ([userid],[reminder])
...
--SET IDENTITY_INSERT [userrmnd] OFF;

--SELECT * FROM [userrmnd]
...
SET IDENTITY_INSERT [val_locs] ON;

INSERT INTO [val_locs] ([val_locsid],[loc_row],[loc_rack],[loc_level],[last_inv])
...
SET IDENTITY_INSERT [val_locs] OFF;

--SELECT * FROM [val_locs]
...
SET IDENTITY_INSERT [vendcomp] ON;

INSERT INTO [vendcomp] ([vendcompid],[componetid],[vendorid],[price],[date],[po_hdrid],[concur],[prim_sec],[skiplot],[mc],[consign],[leadtime],[notes],[lead_wks],[moq],[samplanid])
SELECT [rawUpsize_Contech].[dbo].[vendcomp].[vendcompid]
  ...
  LEFT JOIN [componet] componet ON [rawUpsize_Contech].[dbo].[vendcomp].[comp] = componet.[comp] 
  LEFT JOIN [vendor] vendor ON [rawUpsize_Contech].[dbo].[vendcomp].[ven_id] = vendor.[ven_id]
  LEFT JOIN [po_hdr] po_hdr ON [rawUpsize_Contech].[dbo].[vendcomp].[po_no] = po_hdr.[po_no]
  LEFT JOIN [samplans] samplans ON [rawUpsize_Contech].[dbo].[vendcomp].[po_no] = samplans.[code]
 ORDER BY [rawUpsize_Contech].[dbo].[vendcomp].[vendcompid]

SET IDENTITY_INSERT [vendcomp] OFF;

--SELECT * FROM [vendcomp]
...
INSERT INTO [vendctct] ([ven_id],[first_name],[last_name],[mi],[department],[email],[ext],[phone],[fax],[job_title])
...
SET IDENTITY_INSERT [vendeval] ON;

INSERT INTO [vendeval] ([vendevalid],[vendorid],[eval_date],[eval_code],[poor_work],[poor_service],[fail_dates],[fail_contract],[other],[comments],[add_userid],[add_dt])
SELECT [rawUpsize_Contech].[dbo].[vendeval].[vendevalid]
  ...
  LEFT JOIN [vendor] vendor ON [rawUpsize_Contech].[dbo].[vendeval].[ven_id] = vendor.[ven_id]
  LEFT JOIN [users] users ON [rawUpsize_Contech].[dbo].[vendeval].[add_user] = users.[username]	-- FK = [users].[userid]

SET IDENTITY_INSERT [vendeval] OFF;

--SELECT * FROM [vendeval]
