...
INSERT INTO [pouches] ([bom_hdrid],[pouch],[pouch2],[lot_no],[lot_no2],[lbl1],[lbl2],[lbl_side],[lbl_side2],[insert_],[insert2],[insertlot],[insertlot2],[lbl_unit],[lbl_unit2],[qty_disp],[qty_oship],[lbl_oship],[lbl_disp])
SELECT ISNULL(bom_hdr.[bom_hdrid], 0) as [bom_hdrid]
  ...
  LEFT JOIN [bom_hdr] bom_hdr ON [rawUpsize_Contech].[dbo].[pouches].[bom_no] = bom_hdr.[bom_no] AND [rawUpsize_Contech].[dbo].[pouches].[bom_rev] = bom_hdr.[bom_rev] 
...
SET IDENTITY_INSERT [prdlinbm] ON;

INSERT INTO [prdlinbm] ([prdlinbmid],[prdlinhdid],[bom_hdrid])
SELECT [rawUpsize_Contech].[dbo].[prdlinbm].[prdlinbmid]
  ...
  LEFT JOIN [bom_hdr] bom_hdr ON [rawUpsize_Contech].[dbo].[prdlinbm].[bom_no] = bom_hdr.[bom_no] AND [rawUpsize_Contech].[dbo].[prdlinbm].[bom_rev] = bom_hdr.[bom_rev] 
  ORDER BY [rawUpsize_Contech].[dbo].[prdlinbm].[prdlinbmid]

SET IDENTITY_INSERT [prdlinbm] OFF;

--SELECT * FROM [prdlinbm]
...
-- Column changes:
--  - Set [prdlnqbmid] to be primary key
--  - Added [bom_hdrid] to be foreign key to link [bom_no] & [bom_rev]
--  - Removed columns [bom_no] & [bom_rev]
...
--SET IDENTITY_INSERT [prdlnqbm] ON;

INSERT INTO [prdlnqbm] ([prdlnqhdid],[bom_hdrid])
SELECT --[rawUpsize_Contech].[dbo].[prdlnqbm].[prdlnqbmid]
  ...
  LEFT JOIN [bom_hdr] bom_hdr ON [rawUpsize_Contech].[dbo].[prdlnqbm].[bom_no] = bom_hdr.[bom_no] AND [rawUpsize_Contech].[dbo].[prdlnqbm].[bom_rev] = bom_hdr.[bom_rev] 
  ORDER BY [rawUpsize_Contech].[dbo].[prdlnqbm].[prdlnqbmid]
  
--SET IDENTITY_INSERT [prdlnqbm] OFF;

--SELECT * FROM [prdlnqbm]
...
SET IDENTITY_INSERT [prdlnqdt] ON;

INSERT INTO [prdlnqdt] ([prdlnqdtid],[prdlnqhdid],[componetid],[type],[sort_order],[mod_dt],[userid],[as_of_dt])
SELECT [rawUpsize_Contech].[dbo].[prdlnqdt].[prdlnqdtid]
  ...
  LEFT JOIN [componet] componet ON [rawUpsize_Contech].[dbo].[prdlnqdt].[comp] = componet.[comp] 
  LEFT JOIN [users] users ON [rawUpsize_Contech].[dbo].[prdlnqdt].[user_id] = users.[username]	
  ORDER BY [rawUpsize_Contech].[dbo].[prdlnqdt].[prdlnqdtid]
   
SET IDENTITY_INSERT [prdlnqdt] OFF;

--SELECT * FROM [prdlnqdt]
...
SET IDENTITY_INSERT [prdlnqhd] ON;

INSERT INTO [prdlnqhd] ([prdlnqhdid],[descript],[customerid],[userid],[add_dt])
SELECT [rawUpsize_Contech].[dbo].[prdlnqhd].[prdlnqhdid]
  ...
  LEFT JOIN [customer] customer ON [rawUpsize_Contech].[dbo].[prdlnqhd].[cust_no] = customer.[cust_no] 
  LEFT JOIN [users] users ON [rawUpsize_Contech].[dbo].[prdlnqhd].[user_id] = users.[username]	
  
SET IDENTITY_INSERT [prdlnqhd] OFF;

--SELECT * FROM [prdlnqhd]
